...
description: |
  In this post we will use dynamic factor analysis to investigate  
  the per capita CO2 emissions produced over the last 100 years by 15 countries of different geographical   
   and economical areas. To estimate the models we will use the R package MARSS.
categories:
  - R
  - Econometrics
  - Time series
...
Following further the analysis and considering the log transformed data in Figure \@ref(fig:log2share),
...
```{r, logco2share, fig.cap= "log transform of the global CO2 share"}
...
```{r co2capita, fig.cap= "Time serie plot of the CO2 per capita emissions"}
...
In order to fit our model data need to be normal.
...
```{r states, fig.cap = "Estimate smoothed states", code_folding = TRUE}
...
```{r stateres, fig.cap = "State residuals plot", code_foldin = TRUE}
p1 <- state.res %>% ggplot() +
